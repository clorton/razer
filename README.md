...
For the closed-population menagerie you usually don't wire this by hand: **`run_model(scenario, model, nticks, r0, …)`** builds the population, runs the correct per-tick loop and applies all census deltas, and returns a **`model` environment** (bundling `$people`, `$nodes`, `$network`, `$carry`) with per-node census and the `incidence` / `onset` / `recovery` / `waning` flows — e.g. `run_model(data.frame(population = 1e5, I = 100), "SEIR", nticks = 200, r0 = 2.5, infectious_period = 8, incubation_period = 5, seed = 1)`. It seeds any of the model's `E`/`I`/`R` states the scenario supplies, and takes optional `init` / `step_enter` / `step_exit` callbacks for extension (see `examples/model_callbacks.R`). The hand-wired examples below show what it does under the hood (and how to go beyond the menagerie with vital dynamics, importation, and maternal immunity).
