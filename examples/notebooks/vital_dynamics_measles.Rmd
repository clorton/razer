...
> Companion to `examples/engwal_measles.R`. Builds on the demographics and endemic notebooks.
...
- an **`init` callback** gives each agent an age (from a pyramid-like curve) and a
  Kaplan–Meier date of death (see the demographics notebook).
...
- **Vaccination.** Layer an SIA campaign on top — register `c("M", "V")` and add a campaign
  callback (see the interventions notebook); watch the inter-epidemic period lengthen as
  coverage rises.
