...
## 2. Compare compartment structures

Run four models on the *same* population and parameters; colour by compartment, line type
by model (this is `examples/compare_models.R`, condensed).
...
susceptibles to a new `"V"` state at day 60 (see `examples/sia_campaigns.R` and
`examples/quarantine.R` for scheduled / leaky versions).

```{r vaccinate}
vacc <- run_model(scenario, "SIR", nticks = 200L, r0 = 2.5, infectious_period = 8,
                  seed = 1L, extra_states = "V",
                  step_exit = function(model) {
                    if (model$tick != 60L) return(invisible())
...
matplot(0:199, cbind(baseline = rowSums(m$nodes$I$values()),
                     vaccinated = rowSums(vacc$nodes$I$values())) / 1e3,
        type = "l", lty = 1, lwd = 2, col = c("#e31a1c", "#1f78b4"),
        xlab = "day", ylab = "infectious (thousands)",
        main = "Infectious: no intervention vs a day-60 vaccination pulse")
legend("topright", c("baseline", "day-60 pulse (40% of S -> V)"),
