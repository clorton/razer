...
#   3. step_infectious_ir(people, imm_duration)                  — I→R, sets immunity timer
...
run_seirs <- function(n, n_seed = 200L, beta = 0.5, exp_duration = 3L,
  ...
  for (tick in seq_len(nticks)) {
    ...
    step_infectious_ir(ppl, imm_duration = imm_duration)
    step_recovered_rs(ppl)   # counts down R→S; timer was set to imm_duration by step_infectious_ir
